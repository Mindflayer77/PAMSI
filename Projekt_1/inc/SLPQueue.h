...
template <typename T> bool SLPQueue<T>::remove(const T &elemRmv) {
  ...
  if (isEmpty()) // jesli jest pusta lista to zwracamy false;
    return false;
  if (iter->elem == elemRmv) { // jezeli element jest na poczatku listy
    deQueue();                 // usuwamy pierwszy element listy;
    return true;
  }
  while (iter != back) {               // dopoki nie dojdzie do konca listy
    if (iter->next->elem == elemRmv) { // sprawdzamy element w następnym Node
...
template <typename T>
void SLPQueue<T>::enQueue(const T &newElem, const int &priority) {
